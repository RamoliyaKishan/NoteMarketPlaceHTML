...
if(isset($_SESSION['UsersID'])) {
    ...
    if(isset($_POST['save'])){
		...
		$sell_for = trim($_POST['sell_for']);
		$price = trim($_POST['price']);
        
        
        $display_picture_filename        = trim($_FILES['display_picture']['name']);
        $temp_display_picture_filename   = trim($_FILES['display_picture']['tmp_name']);
            
        $note_filename        = trim($_FILES['note']['name']);
        $temp_note_filename   = trim($_FILES['note']['tmp_name']);
            
        $note_preview_filename        = trim($_FILES['note_preview']['name']);
        $temp_note_preview_filename   = trim($_FILES['note_preview']['tmp_name']);
        ...
        $query = "INSERT INTO seller_notes (SellerID, Status , Title, Category , DisplayPicture, Note, NoteType , NumberofPages, Description, UniversityName, Country, Course, CourseCode, Professor, IsPaid, SellingPrice, NotesPreview) ";
             
        $query .= "VALUES ({$user_id}, 'Draft', '{$title}',  '{$category_id}', '{$display_picture_filename}', '{$note_filename}', '{$type_id}', '{$pages}', '{$description}', '{$institute_name}', '{$country_id}', '{$course}', '{$course_code}', '{$professor}', '{$sell_for}', '{$price}', '{$note_preview_filename}')"; 
        ...
        if(!$add_note_query){
            ...
            $_SESSION['NoteID'] = $note_id;
        }
        
        $publish_button = "";
        
        if(isset($_POST['publish'])) {
            $query = "UPDATE seller_notes SET Status='Submitted' WHERE NoteID = '$note_id'";
								...
								<div class="form-group">
									...
									<div class="select-type">
										<select class="myInput" id="type" name="type" required>
											<option selected disabled>Select tour note type</option>
											<?php
