...
<a class="btn" href="" role="button" id="download-btn" name="download-btn" >DOWNLOAD <?php echo ($db_IsPaid == "Free") ? '' : '$'. $db_SellingPrice ?></a>
                            ...
                            if(isset($_POST['download-btn'])){
                                if(isset($_SESSION['UsersID'])) {
                                    ...
                                    <script>
                                        ...
                                        if(confirm("Do You want to download it ???? ")){
                                            <?php 
                                                $_SESSION['NoteID'] = $db_NoteID;
                                                redirect("buyer-request.php");
                                            ?>
                                        }else {
                                            <?php
                                            redirect("search-notes.html");
                                            ?>
                                        }
...
$query = "SELECT * FROM  user_profile WHERE UsersID = '{$user_id}'";
    ...
    if(isset($_POST['submit_profile'])) {
            ...
            $profile_picture_filename        = trim($_FILES['profile_picture']['name']);
            $temp_profile_picture_filename   = trim($_FILES['profile_picture']['tmp_name']);

            move_uploaded_file($temp_profile_picture_filename, "../profile pictures/".$profile_picture_filename);
            
            if($count > 0){
                
                $query = "UPDATE user_profile SET UsersID = {$user_id}', DOB = '{$birth_date}', Gender = '{$gender}', PhoneCountryCode = '{$country_code}, PhoneNumber = '{$phone_number}', ProfilePicture = '{$profile_picture_filename}', Address1 = '{$address1}', Address2 = '{$address1}''{$address2}', City = '{$city}', State = '{$state}',, ZipCode = '{$zipcode}', Country = '{$country_id}', University = '{$university}', Collage = '{$collage}', ModifiedDate =  now(), ModifiedBy = '{$user_id}' WHERE UsersID = '{$user_id}'"; 
...
if(isset($_GET['clone'])){
                $NoteID = $_GET['clone'];
                $query = "INSERT INTO seller_notes SELECT * FROM seller_notes WHERE NoteID = '{$NoteID}'";
                $select_notes_query = mysqli_query($connection, $query);
                if(!$select_notes_query) {
                    die("QUERY FAILED" . mysqli_error($connection));
                }else {
                    $new_NoteID = mysqli_insert_id($connection);
                }
                
                $query = "UPDATE seller_notes SET NoteID = (SELECT MAX(NoteID)+1 FROM seller_notes), Status = 'Draft', ActionedBy='', AdminRemarks = '' WHERE NoteID = '{$new_NoteID}'";
                $select_notes_query = mysqli_query($connection, $query);
                if(!$select_notes_query) {
                    die("QUERY FAILED" . mysqli_error($connection));
                }
...
$NoteID = $_GET['clone'];
                $mysqli->query("CREATE TEMPORARY TABLE temp_tbl SELECT * seller_notes WHERE NoteID = '{$NoteID}'");
                $mysqli->query("UPDATE temp_tbl SET NoteID = (SELECT MAX(NoteID)+1 FROM seller_notes), Status = 'Draft', ActionedBy='', AdminRemarks = '' ");
                $mysqli->query("INSERT INTO seller_notes SELECT * temp_tbl");
                $mysqli->query("DROP TABLE temp_tbl");
...
$("#give_review").click(function(){
         ...
         if(isset($_POST['publish'])) {
                    ...
                    $query = "UPDATE seller_notes SET Status='Submited for Review' WHERE NoteID = '{$NoteID}'";
                    $update_status_query = mysqli_query($connection, $query);

                        // mail to admin for check published notes
                    $query = "SELECT Value FROM system_configurations WHERE KeyName = 'AdminEmail' ";
