<?php include "db.php"; 
    ...
    if(isset($_POST['action'])){
        $query = "SELECT * FROM seller_notes WHERE Status = 'Published' AND IsActive = '1'";
        $output = "";
        ...
        if($count_row > 0) {
            while($row = mysqli_fetch_array($result)){
                ...
                if(empty($db_DisplayPicture)){
                    $query = "SELECT Value FROM  system_configuration WHERE KeyName = 'DefaultNotePicture'";    
                    $default_query = mysqli_query($connection, $query);
                    $db_DisplayPicture = $row['Value'];
                }
