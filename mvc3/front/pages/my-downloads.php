			...
			<div class="container">
				...
				<table id="my-downloads-table" class="datatable table table-responsive display">
                    ...
                    <tbody>
                        ...
                        $query = "SELECT * FROM downloads WHERE DownloaderID = '{$user_id}' AND (IsSellerHasAllowedDownload = '1' OR IsPaid = '0') ORDER BY ModifiedDate DESC";
                        ...
                        <?php
                            ...
                            if(isset($_GET['d_id'])){
                                ...
                                $db_DownloadID = $_GET['d_id'];
                                
                                $query = "SELECT sn.Note FROM seller_notes AS sn INNER JOIN downloads AS d ON d.NoteID = sn.NoteID WHERE DownloadID = '{$db_DownloadID}'";
                                ...
                                $query = "SELECT IsAttachmentDownloaded FROM downloads WHERE DownloadID = '{$db_DownloadID}' AND  IsAttachmentDownloaded = '1'";
                                $select_downloades_query = mysqli_query($connection, $query);    
                                $count = mysqli_num_rows($select_downloades_query);
                                
                                if($count <= 0){
                                    $query = "UPDATE downloads SET AttachmentPath = '{$filePath}', IsAttachmentDownloaded = '1', AttachmentDownloadedDate = now() WHERE DownloadID = '{$db_DownloadID}'";
                                    $update_downloades_query = mysqli_query($connection, $query);
                            ...
                            if(isset($_POST['give_review'])){
                                ...
                                $noteId = escape($_POST['Review_model_noteid']);                                
